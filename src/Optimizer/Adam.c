...
Adam_ptr create_adam(const double learning_rate, const double eta_decrease, const double beta1, const double beta2, const double epsilon) {
    ...
    result->sgd.optimizer.learning_rate = learning_rate;
    result->sgd.optimizer.eta_decrease = eta_decrease;
    result->sgd.optimizer.set_gradients = set_gradients_adam;
    result->sgd.momentum = beta1;
    result->sgd.velocity_map = create_hash_map((unsigned int (*)(const void *, int)) hash_function_computational_node,
    (int (*)(const void *, const void *)) compare_computational_node);
    result->beta2 = beta2;
    result->epsilon = epsilon;
    result->momentum_map = create_hash_map((unsigned int (*)(const void *, int)) hash_function_computational_node,
        (int (*)(const void *, const void *)) compare_computational_node);
    return result;
}
...
void set_gradients_adam(void *a, Computational_node_ptr node) {
    ...
    for (int i = 0; i < node->backward->total_elements; i++) {
        new_values_momentum[i] /= 1 - adam->sgd.momentum;
        new_values_velocity[i] /= 1 - adam->beta2;
    }
